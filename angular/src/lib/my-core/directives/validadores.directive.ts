...
export function EsFechaNoPasada(): ValidatorFn {
  return (control: AbstractControl): { [key: string]: any } | null => {
    const dateNow: Date = new Date();
    const dateFuture = control.value.getTime();
      if(dateFuture > dateNow) {
        console.log(dateFuture);
        return null;
      }
      else
        return { esfechanopasada: 'Tiene que ser una fecha futura' }
  };
}

@Directive({
  selector: '[esfechanopasada][formControlName],[esfechanopasada][formControl],[esfechanopasada][ngModel]',
  providers: [{ provide: NG_VALIDATORS, useExisting: EsFechaNoPasadaValidator, multi: true }]
})
export class EsFechaNoPasadaValidator implements Validator {
  validate(control: AbstractControl): ValidationErrors | null {
      return EsFechaNoPasada()(control);
  }
}
